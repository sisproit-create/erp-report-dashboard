...
function App() {
  ...
  async function load() {
    ...
    const validReports=(safe<Report[]>(r) ?? [])
      ...
      .sort((a,b)=>{
        const aTime=new Date(a.created_at || a.fecha_publicacion).getTime();
        const bTime=new Date(b.created_at || b.fecha_publicacion).getTime();
...
function Interpretation({title,summary,points,action}:{title:string;summary:string;points:string[];action:string}) {
  return <section className="interpretation-card">
    <div className="interpretation-icon"><Activity size={20}/></div>
    <div className="interpretation-copy"><span>LECTURA GERENCIAL</span><h3>{title}</h3><p>{summary}</p><ul>{points.map((point,index)=><li key={index}>{point}</li>)}</ul><strong>Acción recomendada: {action}</strong></div>
  </section>;
}

function Production({summary:s,series}:{summary:Summary|null;series:SeriesRow[]}) { return <>
  <div className="metrics-grid four"><Metric label="Producción mes" value={`${fmt.format(s?.total_ton||0)} t`} icon={Factory}/><Metric label="Producciones" value={fmt.format(s?.producciones||0)} icon={BarChart3}/><Metric label="Promedio corrida" value={`${fmt.format(s?.produccion_promedio||0)} t`} icon={Gauge}/><Metric label="Variabilidad" value={`${fmt.format(s?.variacion_pct||0)}%`} icon={Activity}/></div>
  <Interpretation title="¿Cómo se está comportando la producción?" summary={`Durante el periodo se registraron ${fmt.format(s?.total_ton||0)} toneladas distribuidas en ${fmt.format(s?.producciones||0)} corridas.`} points={[`El promedio por corrida es ${fmt.format(s?.produccion_promedio||0)} toneladas.`,`La variabilidad registrada es ${fmt.format(s?.variacion_pct||0)}%; debe leerse junto con paradas, tipo de mezcla y jornada.`,`Una mayor producción es positiva únicamente cuando el costo/T y el consumo energético permanecen controlados.`]} action="comparar los días de mayor tonelaje con diésel gal/T, AC30 kg/T y costo por tonelada."/>
  <Panel title="Producción diaria" kicker="TONELADAS"><ResponsiveContainer width="100%" height={420}><AreaChart data={series}><defs><linearGradient id="prod" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#18b9f2" stopOpacity={.4}/><stop offset="1" stopColor="#18b9f2" stopOpacity={.02}/></linearGradient></defs><CartesianGrid stroke="#1e3345" vertical={false}/><XAxis dataKey="fecha" tick={{fill:'#7890a5'}}/><YAxis tick={{fill:'#7890a5'}}/><Tooltip content={<ChartTooltip/>}/><Area dataKey="toneladas" name="Toneladas" stroke="#18b9f2" fill="url(#prod)" strokeWidth={3}/></AreaChart></ResponsiveContainer></Panel>
</> }

function FuelPage({summary:s,series,equipment}:{summary:Summary|null;series:SeriesRow[];equipment:EquipmentRow[]}) { return <>
  <div className="metrics-grid four"><Metric label="Diésel planta" value={`${fmt.format(s?.diesel_planta||0)} gal`} icon={Fuel}/><Metric label="Apoyo DMI" value={`${fmt.format(s?.diesel_apoyo||0)} gal`} icon={Truck}/><Metric label="Transferencias" value={`${fmt.format(s?.transferencia_interna||0)} gal`} icon={Activity}/><Metric label="Eficiencia" value={`${fmt.format(s?.diesel_ton||0)} gal/T`} icon={Gauge}/></div>
  <Interpretation title="¿Dónde se está consumiendo el combustible?" summary={`La eficiencia consolidada es ${fmt.format(s?.diesel_ton||0)} galones por tonelada.`} points={[`La planta registra ${fmt.format(s?.diesel_planta||0)} galones y los equipos de apoyo ${fmt.format(s?.diesel_apoyo||0)} galones.`,`Las transferencias internas (${fmt.format(s?.transferencia_interna||0)} gal) no deben contarse dos veces como consumo.`,`Un aumento de gal/T puede indicar calentamiento prolongado, baja producción, paradas o asignaciones incompletas.`]} action="revisar los días con mayor gal/T y conciliarlos con horas de operación y despachos HYUNDAI."/>
  <div className="dashboard-grid equal"><Panel title="Diésel gal/T" kicker="TENDENCIA"><MiniLine title="Eficiencia diaria" data={series} keyName="diesel_gal_ton" unit="gal/T"/></Panel><Panel title="Consumo por equipo" kicker="HYUNDAI"><EquipmentChart data={equipment}/></Panel></div>
</> }

function AC30Page({summary:s,series}:{summary:Summary|null;series:SeriesRow[]}) { return <>
  <div className="metrics-grid four"><Metric label="AC30 mes" value={`${fmt.format(s?.ac30_kg||0)} kg`} icon={Droplets}/><Metric label="AC30 por tonelada" value={`${fmt.format(s?.ac30_kg_ton||0)} kg/T`} icon={Gauge}/><Metric label="Laboratorio" value={`${fmt.format(s?.lab_ac30_promedio||0)}%`} sub={`${s?.lab_muestras||0} muestras`} icon={ShieldCheck}/><Metric label="Vacíos promedio" value={`${fmt.format(s?.lab_vacios_promedio||0)}%`} icon={Activity}/></div>
  <Interpretation title="¿La dosificación está alineada con el diseño?" summary={`El consumo promedio es ${fmt.format(s?.ac30_kg_ton||0)} kg/T y el laboratorio reporta ${fmt.format(s?.lab_ac30_promedio||0)}% de AC30.`} points={[`El análisis debe comparar dosificación real, fórmula de diseño y tipo de mezcla.`,`Los vacíos promedio (${fmt.format(s?.lab_vacios_promedio||0)}%) deben evaluarse junto con Gmb, Gmm, VMA y VFA.`,`Las variaciones pueden estar relacionadas con calibración, humedad, flujo de bomba o ajustes de producción.`]} action="validar cualquier desviación diaria contra la fórmula aprobada y los resultados de laboratorio."/>
  <Panel title="Dosificación AC30 por tonelada" kicker="TENDENCIA"><ResponsiveContainer width="100%" height={420}><LineChart data={series}><CartesianGrid stroke="#1e3345" vertical={false}/><XAxis dataKey="fecha" tick={{fill:'#7890a5'}}/><YAxis tick={{fill:'#7890a5'}}/><Tooltip content={<ChartTooltip/>}/><Line dataKey="ac30_kg_ton" name="AC30 kg/T" stroke="#2dd4bf" strokeWidth={3}/></LineChart></ResponsiveContainer></Panel>
</> }

function EquipmentPage({equipment}:{equipment:EquipmentRow[]}) { const top=equipment[0]; return <>
  <Interpretation title="¿Qué equipos concentran el consumo?" summary={top?`${top.equipo} aparece como el mayor consumidor del periodo con ${fmt.format(top.galones_consumo)} galones.`:'Todavía no hay consumos de equipos publicados para el periodo.'} points={[`El ranking debe compararse con horas trabajadas y actividad real.`,`Consumo sin actividad registrada puede indicar captura incompleta o una asignación pendiente.`,`Las transferencias deben conciliarse antes de evaluar el costo operativo final.`]} action="validar los tres equipos de mayor consumo y documentar cualquier diferencia frente a sus horas de trabajo."/>
  <Panel title="Consumo operativo HYUNDAI por equipo" kicker="DISTRIBUCIÓN"><EquipmentChart data={equipment}/></Panel><Panel title="Detalle mensual" kicker="EQUIPOS"><div className="table-wrap"><table><thead><tr><th>Equipo</th><th>Consumo</th><th>Transferencia</th><th>Costo</th></tr></thead><tbody>{equipment.map(e=><tr key={e.codigo}><td>{e.equipo}</td><td>{fmt.format(e.galones_consumo)} gal</td><td>{fmt.format(e.galones_transferencia)} gal</td><td>{money.format(e.costo_diesel_usd)}</td></tr>)}</tbody></table></div></Panel>
</> }

function CostsPage({summary:s,series,rankings}:{summary:Summary|null;series:SeriesRow[];rankings:RankingRow[]}) { return <>
  <div className="metrics-grid four"><Metric label="Costo/T gerencial" value={`${money.format(s?.costo_total_ton||0)}/T`} icon={CircleDollarSign}/><Metric label="Costo mensual" value={money.format(s?.costo_total_mes||0)} icon={CircleDollarSign}/><Metric label="Margen estimado" value={`${fmt.format(s?.margen_pct||0)}%`} icon={TrendingUp}/><Metric label="Variabilidad" value={`${fmt.format(s?.variacion_pct||0)}%`} icon={Activity}/></div>
  <Interpretation title="¿Qué está ocurriendo con el costo y el margen?" summary={`El costo gerencial se ubica en ${money.format(s?.costo_total_ton||0)}/T y el margen estimado en ${fmt.format(s?.margen_pct||0)}%.`} points={[`La variabilidad del costo/T es ${fmt.format(s?.variacion_pct||0)}%.`,`Una jornada económica solo es eficiente si también cumple diseño, laboratorio y calidad.`,`El ranking ayuda a detectar qué combinaciones de tonelaje, AC30 y diésel explican los mejores y peores resultados.`]} action="abrir las jornadas de mayor costo y comparar insumos, producción, paradas y cumplimiento técnico."/>
  <Panel title="Costo por tonelada" kicker="TENDENCIA"><MiniLine title="Costo/T diario" data={series} keyName="costo_ton" unit="USD/T"/></Panel><div className="dashboard-grid equal"><Ranking title="Top eficientes" rows={rankings.filter(r=>r.tipo_ranking==='eficiente')}/><Ranking title="Mayor costo" rows={rankings.filter(r=>r.tipo_ranking==='mayor_costo')}/></div>
</> }

function AlertsPage({alerts}:{alerts:AlertRow[]}) { return <>
  <Interpretation title="¿Qué requiere atención inmediata?" summary={alerts.length?`El sistema presenta ${alerts.length} alertas o condiciones pendientes de validación.`:'No se registran alertas para el periodo seleccionado.'} points={[`Una alerta indica que una regla o umbral fue superado; no confirma por sí sola una falla.`,`Las alertas críticas deben validarse primero y documentar su cierre.`,`Las advertencias repetitivas pueden revelar una tendencia operativa antes de convertirse en un problema mayor.`]} action="priorizar por severidad, asignar responsable y registrar la evidencia del cierre."/>
